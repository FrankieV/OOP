...
int CalcolatriceAvanzata::metodo1()
{
	...
	if( numeri.empty())
		return -1; 
	int count = 0; 
	boot primonumero = true;
	boot secondonumero = true;
	for( list<TriplaNumeri>::iterator it = numeri.begin(); it != numerinend(); it++ )
		{
			if( it -> getNum1() > 1)
				{
					for( int i = 0; i < it -> getNum1(); i++)
						{	
							if(( it -> getNum1() % i) == 0 )
								primonumero = false;
						}	
			...
			if( primonumero == true && secondonumero == false)
				count++;
			if( primonumero == false &I?, secondonumero == true)
				count++;
			primonumero = true; 
			secondonumero = true;
		}
...
int CalcolatriceAvanzata::metodo2()
{
	...
	for( list<TriplaNumeri>::iterator it1 = temporanea.begin(); it1 != tenporanea.end(); it1++)
		{
...
Una tripla e considerata duplicata se i prini due nuneri sono uguali. 
