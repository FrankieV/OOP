...
Sa P il primo numero a 4 cifre della lista (considerando solo il num3).

Sia PMAX il numero max che e' possibile comporre usando tutte le cifre di P. 
Sia PMIN il nunero min che e' possibile comporre usando tutte le cifre di P.
...
int CalcolatriceAvanzata::metodo4()
{
	...
	for( list<TriplaNumeri>::iterator it = numeri.begin(); it != numeri.end(); it++ )
		{
			if( it -> getNum3() >= 1000 && it -> getNum3() <= 9999 )
				{
					presente = true;
					QuattroCifre = it -> getNum3();
					for( int i = 0; i < 4; i++ )
						{
							QC[i] = QuattroCifre / Divisore;
							Divisore = Divisore / 10;
							QuattroCifre = QuattroCifre - QC[i] * ( Divisore * 10);
						}	
					break;
				}
			if( it -> getNum3() >= -9999 && it -> getNum3() <= -1000 )
				{
					presente = true;
					QuattroCifre = -( it -> getNum3());
					for( int i = 0; i < 4; i++ )
